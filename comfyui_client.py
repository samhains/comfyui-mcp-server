...
class ComfyUIClient:
    ...
    def generate_i2v(self, image_url, prompt, width=None, height=None, frame_length=None):
        """Generate video from a single image using WAN 2.2 dual-CFG distillation (fast, no audio)

        Args:
            image_url (str): URL to the input image
            prompt (str): Motion/scene description for the video
            width (int, optional): Video width in pixels. Defaults to 640.
            height (int, optional): Video height in pixels. Defaults to 640.
            frame_length (int, optional): Number of frames. Defaults to 81.

        Returns:
            str: URL to the generated video file
        """
        try:
            workflow_id = "wan2.2-i2v-dual-cfg"
            ...
            params = {"image_url": image_url, "prompt": prompt}
            if width is not None:
                params["width"] = width
            if height is not None:
                params["height"] = height
            ...
            logger.info(f"Submitting i2v workflow {workflow_id} to ComfyUI...")
            response = requests.post(f"{self.base_url}/prompt", json={"prompt": workflow})
            if response.status_code != 200:
                raise Exception(f"Failed to queue workflow: {response.status_code} - {response.text}")

            prompt_id = response.json()["prompt_id"]
            logger.info(f"Queued i2v workflow with prompt_id: {prompt_id}")

            while True:
                history = requests.get(f"{self.base_url}/history/{prompt_id}").json()
                if history.get(prompt_id):
                    outputs = history[prompt_id]["outputs"]
                    logger.info("I2V workflow outputs: %s", json.dumps(outputs, indent=2))
                    video_url = _extract_output_url(self.base_url, outputs, workflow_id)
    ...
    def generate_i2v_maudio(self, image_url, width=None, height=None, frame_length=None):
        """Generate video with audio from a single image using WAN 2.2 I2V + MMAudio
        ...
        try:
            ...
            logger.info(f"Submitting i2v maudio workflow {workflow_id} to ComfyUI...")
            response = requests.post(f"{self.base_url}/prompt", json={"prompt": workflow})
            if response.status_code != 200:
                raise Exception(f"Failed to queue workflow: {response.status_code} - {response.text}")

            prompt_id = response.json()["prompt_id"]
            logger.info(f"Queued i2v maudio workflow with prompt_id: {prompt_id}")

            while True:
                history = requests.get(f"{self.base_url}/history/{prompt_id}").json()
                if history.get(prompt_id):
                    outputs = history[prompt_id]["outputs"]
                    logger.info("I2V maudio workflow outputs: %s", json.dumps(outputs, indent=2))
                    video_url = _extract_output_url(self.base_url, outputs, workflow_id)
                    logger.info(f"Generated i2v maudio video URL: {video_url}")
                    return video_url
                time.sleep(1)

        except Exception as e:
            raise Exception(f"Error generating i2v maudio video: {e}")
